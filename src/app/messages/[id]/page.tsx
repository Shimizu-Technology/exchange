...
export default function ChatPage() {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-8">
        <div className="animate-pulse space-y-4">
          <div className="h-16 bg-charcoal/5 rounded-xl" />
          <div className="h-[60vh] bg-charcoal/5 rounded-xl" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-xl mx-auto flex flex-col h-[calc(100vh-4rem)] md:h-[calc(100vh-5rem)]">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 border-b border-charcoal/5 bg-white/80 backdrop-blur-sm">
        <button onClick={() => router.push("/messages")} className="text-muted hover:text-charcoal transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          {listing && (
            <Link href={`/listing/${listing._id}`} className="text-xs text-coral truncate block hover:underline">
      ...
      {listing && (
        <Link
          ...
          className="flex items-center gap-3 px-4 py-2.5 bg-cream border-b border-charcoal/5 hover:bg-white transition-colors"
        >
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-coral/10 to-sage/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
            {listing.photos[0] ? (
              <img src={listing.photos[0]} alt="" className="w-10 h-10 object-cover" />
            ) : (
              <Heart className="w-4 h-4 text-coral/30" />
            )}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium truncate">{listing.title}</p>
            <p className="text-xs text-coral font-bold">{formatPrice(listing.price)}</p>
          </div>
        </Link>
      )}
        ...
        {messages?.length === 0 && (
          <div className="text-center py-8">
            ...
            <button
              ...
              className="mt-3 px-4 py-2 bg-white rounded-full border border-charcoal/10 text-sm text-muted hover:text-coral hover:border-coral/20 transition-colors"
            >
            ...
            >
              <div className={`max-w-[80%] px-4 py-2.5 rounded-2xl ${
                isMe
                  ...
                  : "bg-white border border-charcoal/5 rounded-bl-md"
              }`}>
      ...
      {/* Input */}
      <div className="px-4 py-3 border-t border-charcoal/5 bg-white">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 bg-cream rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-coral/10"
          />
          <button
            onClick={handleSend}
            disabled={!text.trim()}
            className="p-3 bg-coral text-white rounded-2xl hover:bg-coral-dark disabled:opacity-30 transition-all"
          >
